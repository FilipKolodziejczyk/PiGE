...
COLORREF		bgColor = RGB(255, 255, 0);
INT				mode = SOLID_MODE;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		hOldBitmap = (HBITMAP)SelectObject(offDC, hBitmap);
		ReleaseDC(hWnd, hdc);
	}
	break;
...
LRESULT CALLBACK PaddleProc(HWND hPaddle, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		wchar_t s[MAX_DIGITS + 1];
		_itow_s(points, s, MAX_DIGITS);
		s[MAX_DIGITS] = '\0';
		RECT rc;
		GetClientRect(hPaddle, &rc);
		SetTextColor(hdc, textColor);
		SetBkMode(hdc, TRANSPARENT);
		DrawTextW(hdc, s, (int)_tcslen(s), &rc, DT_CENTER);
		EndPaint(hWnd, &ps);
	}
	break;
